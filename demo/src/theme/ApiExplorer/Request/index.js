...
const slice_1 = require("../Response/slice");
...
function Request({ item }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await delay(1200);
      //store the request in local storage with
      const res = await (0, makeRequest_1.default)(postmanRequest, proxy, body);
      //get time of response in format like 3/29/2022 12:00:00 PM
      const timeOfResponse = new Date().toLocaleString("en-US", {
        month: "numeric",
        day: "numeric",
        year: "numeric",
        hour: "numeric",
      ...
      existingResponses.push(Obj);
      localStorage.setItem("responses", JSON.stringify(existingResponses));
      dispatch((0, slice_1.setResponses)(existingResponses));
      dispatch((0, slice_1.setResponse)(data));
